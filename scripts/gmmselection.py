...
def process_all_dfs(result_dict, viro3dclusters):
    ...
    for key, df in result_dict.items():
        ...
        if before != after:
            print(
                f"⚠️ Row count changed during merge: before = {before}, after = {after}"
            )
        else:
        ...
        for cluster_id in df["cluster_id"].dropna().unique():
            ...
            for feature_combination in feature_combinations:
                ...
                if clustering_successful:
                    model_id = (
                        f"{key}_Cluster_{cluster_id}_"
                        f"{'_'.join(feature_combination)}_cov-tied"
                    )
    ...
    for model_id, result_data in all_gmm_results.items():
        ...
        for cluster_num in unique_clusters:
            if cluster_num not in cluster_colors:
                cluster_colors[cluster_num] = OTHER_COLORS[
                    color_idx % len(OTHER_COLORS)
                ]
                color_idx += 1
        ...
        for cluster_num in unique_clusters:
            ...
            for _, row in cluster_df.iterrows():
                ...
                if "host_gene_names_primary" in row and pd.notna(
                    row["host_gene_names_primary"]
                ):
            ...
            cluster_label = f"Cluster {cluster_num}" + (
                f" (Best {best_metric})" if is_best else ""
            )
        ...
        fig.update_layout(
            scene=dict(
                xaxis=dict(
                    title=dict(
                        text="Alignment length", font=dict(size=15)
                    ),
                    range=[0, 650],
                ...
                yaxis=dict(
                    title=dict(
                        text="Neg log E-value", font=dict(size=15)
                    ),
                    range=[0, 25],
                    tickfont=dict(size=13),
                ),
                zaxis=dict(
                    title=dict(
                        text="Query TM-score", font=dict(size=15)
                    ),
                    range=[0, 1],
                    tickfont=dict(size=13),
                ),
