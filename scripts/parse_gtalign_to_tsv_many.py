...
def parse_gtalign_output(input_file):
    ...
    for i, line in enumerate(lines):
        ...
        # any number of digits, a space, and then a string containing "pdb" (so will only work for
        # pdb files).
        ...
        # Extract alignment details lower in the file
            ...
            identities_match = re.search(
                r"Identities = (\d+/\d+) \((\d+%)\), Matched = (\d+/\d+) \((\d+%)\), "
                r"Gaps = (\d+/\d+) \((\d+%)\)",
                line,
            )
            if identities_match:
            ...
            if twotm_match:
                if current_no not in alignment_info:
                    alignment_info[current_no] = {}
                alignment_info[current_no].update(
                    {"q2tm": twotm_match.group(2), "t2tm": twotm_match.group(1)}
                )
        elif "d0" in line:
            ...
            if d0_match:
                if current_no not in alignment_info:
                    alignment_info[current_no] = {}
                alignment_info[current_no].update(
                    {"td0": d0_match.group(1), "qd0": d0_match.group(2)}
                )
    ...
    if not data_lines:
        raise ValueError(
            "Could not find valid data lines in the input file. Please ensure the file contains "
