        ...
        if json_file.endswith(".json"):
            ...
            with open(json_path, 'r') as f:
                ...
                for protein in data.get('protein_structures', []):
                    ...
                    genbank_name_curated = protein.get('genbank_name_curated', "No curated name found")
...
def find_refseq_pdb_file(refseq_id, viral_structures_dir):
    ...
    for root, _, files in os.walk(viral_structures_dir):
        for file in files:
...
def main(args):
    ...
    refseq_ids = []
    with open(args.random1000_refseq_list, 'r') as f:
    ...
    for refseq_id in refseq_ids:
        ...
        genbank_entry_name = genbank_mapping.get(genbank_id, "No curated name found") if genbank_id != "No Match" else "No Match"
        output_data.append([refseq_id, genbank_id, refseq_pdb_filename, genbank_entry_name])

    # Save the output table
    output_df = pd.DataFrame(output_data, columns=["RefSeq_ID", "GenBank_ID", "RefSeq_PDB_Filename", "GenBank_Entry_Name"])
...
if __name__ == "__main__":
    import fnmatch
    parser = argparse.ArgumentParser(description="Create a Random900 table with additional details.")
    parser.add_argument("--random1000_refseq_list", required=True, help="Path to the random1000 refseq list.")
    parser.add_argument("--mapping_file", required=True, help="Path to RefSeq-to-GenBank mapping file.")
    parser.add_argument("--viro3d_metadata_dir", required=True, help="Path to the Viro3D metadata directory.")
    parser.add_argument("--viral_structures_dir", required=True, help="Path to the viral structures directory.")
