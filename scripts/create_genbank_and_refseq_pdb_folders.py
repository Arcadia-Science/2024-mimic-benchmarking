...
def main(args):
    ...
    for i, refseq_output in enumerate(args.refseq_subsets):
        ...
        additional_genbank_ids = random.sample(remaining_genbank_ids, max(0, random_size - len(cumulative_genbank_ids)))
        cumulative_genbank_ids.update(additional_genbank_ids)

        # Write GenBank list to file
            ...
            for root, _, files in os.walk(args.viro3d_dir):
                for file in files:
                    if f"{genbank_id}_" in file:
                        shutil.copy(os.path.join(root, file), os.path.join(genbank_subset_dir, file))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--genbank_lists", nargs="+", required=True, help="Paths to GenBank ID lists.")
    parser.add_argument("--genbank_subset_dirs", nargs="+", required=True, help="Paths to output GenBank PDB subset directories.")
    parser.add_argument("--refseq_subset_dirs", nargs="+", required=True, help="Paths to output RefSeq PDB subset directories.")
    parser.add_argument("--refseq_subsets", nargs="+", required=True, help="Paths to output RefSeq ID lists.")
    parser.add_argument("--random_sizes", nargs="+", type=int, required=True, help="Subset sizes for random GenBank IDs.")
    parser.add_argument("--mapping_file", required=True, help="Path to RefSeq-to-GenBank mapping file.")
    parser.add_argument("--viro3d_dir", required=True, help="Directory containing Viro3D PDBs.")
    parser.add_argument("--viral_structures_dir", required=True, help="Directory containing viral structures.")
